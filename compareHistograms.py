...
def create_dif(base_file_path, pr_file_path, pr_number, test_number, cmssw_version, num_processes, output_dir_path, args):
   ...
   base_output_filename = get_output_filename(base_file_path, pr_number, test_number, cmssw_version, False, args.ref_target)
   pr_output_filename = get_output_filename(pr_file_path, pr_number, test_number, cmssw_version, True, args.new_target)
...
def get_output_filename(input_file_path, pr_number, test_number, cmssw_version, isPr, special_name):
   ...
   return 'DQM_V0001_%s%s.root' % (run, special_name)
...
if __name__ == '__main__':
   ...
   parser.add_argument('--new-target', help='PR file to be saved with this name', default='CMSSW_X_X_X-123x_dataRun3_new_v1')
   parser.add_argument('--ref-target', help='Base file to be saved with this name', default='CMSSW_X_X_X-123x_dataRun3_ref_v1')
   args = parser.parse_args()

   cmssw_version = '_'.join(args.release_format.split('_')[:4])

   create_dif(args.base_file, args.pr_file, args.pr_number, args.test_number, cmssw_version, args.num_processes, args.output_dir, args)
